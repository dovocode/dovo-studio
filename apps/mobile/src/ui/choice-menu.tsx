...
export function ChoiceMenu(props: ChoiceProps) {
  ...
  return (
    ...
    >
      <Host ignoreSafeArea="all" colorScheme="dark" style={{ width: '100%', height }}>
        <Menu
          testID={props.label}
          modifiers={[
            buttonStyle('plain'),
          ...
          label={
            <HStack
              spacing={6}
              modifiers={[
                padding({ horizontal: props.compact ? 4 : 12 }),
                frame({ width, height, alignment: 'leading' }),
                contentShape(shapes.rectangle(), ['interaction', 'accessibility']),
              ]}
            >
              <Text
                modifiers={[
                  font({ textStyle: props.compact ? 'subheadline' : 'body', weight: 'medium' }),
