...
export function conversationMessages(task: Task, events: ToolEvents): ThreadMessageLike[] {
  ...
  return task.messages.map((message) => {
    ...
    if (message.attachments?.length)
      content.push({ type: 'data', name: 'dovo.attachments', data: message.attachments })
    ...
    if (turn) {
      ...
      if (reasoning.length) content.push({ type: 'data', name: 'dovo.reasoning', data: reasoning })
        ...
        ? {
            status:
              turn?.status === 'running'
                ...
                : turn?.status === 'failed'
                  ? { type: 'incomplete' as const, reason: 'error' as const, error: turn.error }
...
function toolIdentity(payload: string) {
  try {
    return z.object({ toolId: z.string() }).safeParse(JSON.parse(payload)).data?.toolId
  } catch {
    return undefined
  }
}
