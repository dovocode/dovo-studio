...
describe('mobile thread status', () => {
  it('retains review, failure and stopped state after a turn finishes', () => {
    ...
    expect(taskRowStatus({ ...task, status: 'failed' }, false, true, now)).toBe('Failed · 4m')
    expect(taskRowStatus({ ...task, status: 'cancelled' }, false, true, now)).toBe('Stopped · 4m')
  })
  it('prioritizes input requests and marks cached working state', () => {
    expect(taskRowStatus(task, true, true, now)).toBe('Needs input')
    expect(taskRowStatus({ ...task, status: 'running' }, false, false, now)).toBe(
      'Was working · 8m',
    )
  })
  it('keeps draft and snoozed tasks explicit', () => {
    expect(taskRowStatus({ ...task, status: 'draft', turns: [] }, false, true, now)).toBe('Draft')
    expect(taskRowStatus({ ...task, snoozedUntil: '2026-09-21T10:00:00Z' }, false, true, now)).toBe(
      'Snoozed · 4m',
    )
  })
  it('shows Done only until the latest completed turn has been viewed', () => {
    const unread = { ...task, lastViewedTurnId: undefined }
    ...
    const next = { ...task, turns: [{ ...task.turns![0], id: 'next' }] }
  ...
  it('never lets unread completion hide input, snooze, failure, cancellation or settled state', () => {
    ...
    for (const hidden of [
      { ...unread, archived: true },
      { ...unread, snoozedUntil: '2026-09-21T10:00:00Z' },
      { ...unread, status: 'failed' as const },
      { ...unread, status: 'cancelled' as const },
      { ...unread, status: 'running' as const },
    ])
      expect(showTaskDone(hidden, false, now)).toBe(false)
    expect(taskRowStatus({ ...unread, status: 'failed' }, false, true, now)).toBe('Failed · 4m')
  })
})
