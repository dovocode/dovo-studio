...
export function useTaskViewed(task: Task, chatVisible: boolean) {
  ...
  const { call, connected, connection, activeId, profile, snapshot } = useRuntime()
  ...
  const lastAttempt = useRef<{ key: string; attempt: number } | null>(null)
  const [state, setState] = useState<{ key: string; error: boolean; busy: boolean }>({
    key,
    error: false,
    busy: false,
  })
  useEffect(() => {
    ...
    lastAttempt.current = { key, attempt }
    if (!turnId || AppState.currentState !== 'active') return
    let current = true
    setState({ key, error: false, busy: true })
    void call('/api/tasks/viewed', { id: task.id, turnId, expectedRevision }, responses.ok)
      .then(() => {
        if (current) setState({ key, error: false, busy: false })
      })
      .catch(() => {
        if (current) setState({ key, error: true, busy: false })
      })
    return () => {
      current = false
    }
