...
import { activityIdentity, pendingActivity, type TaskToolEvent } from './task-tool-events'
    ...
    <View style={{ gap: 2 }}>
      ...
      >
        ...
        <Text
          ...
          style={[styles.muted, { flex: 1, fontSize: 13, lineHeight: 18 }]}
        >
...
export function ReasoningActivity({ events }: { events: TaskToolEvent[] }) {
  ...
  return (
    <View style={{ gap: 2 }}>
      ...
      {open && (
        <View style={{ paddingLeft: 22, paddingBottom: 8, gap: 8 }}>
          {events.map((event) => {
            const detail = toolPresentation(event.payload, event.summary, event.inputPayload)
            return (
              <Markdown
