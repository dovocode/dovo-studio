...
it('browses Jira without any code projects and keeps pipeline sources repository-only', () => {
  ...
  expect(workSourceInput(sources[0])).toEqual({ jiraSourceId: 'general' })
  ...
  const updated = {
    ...
    snapshot: {
      ...
      workspace: {
        ...
        jiraIssueLinks: [{ sourceId: 'general', issueId: 'TEAM-42', repositoryId: 'code' }],
      },
    },
  }
  const after = workSources([updated], 'issues')
  expect(workSourceIdentity(before)).toBe(workSourceIdentity(after))
  expect(retainWorkPages([{ source: before[0], items: ['TEAM-42'] }], after)[0]?.items).toEqual([
    'TEAM-42',
  ])
})
it('retains saved Jira issues offline but drops removed or reauthenticated sources', () => {
  const entry = overview('laptop')
  const before = workSources([entry], 'issues')
  const pages = [{ source: before[0], items: ['TEAM-42'] }]
  expect(
    retainWorkPages(pages, workSources([{ ...entry, connected: false }], 'issues'))[0]?.source
      .connected,
  ).toBe(false)
  expect(retainWorkPages(pages, [])).toEqual([])
  const replaced = {
    ...entry,
    profile: { ...entry.profile, connection: { ...entry.profile.connection, token: 'new-token' } },
  }
...
it('keeps Jira cache separate from repository cache and namespace changes', () => {
  const keys = [
    workCacheKey(jira, 'issues', 'detail', { id: 'TEAM-42' }),
    workCacheKey({ ...jira, site: 'https://other.atlassian.net' }, 'issues', 'detail', {
      id: 'TEAM-42',
    }),
    workCacheKey({ ...jira, project: 'OTHER' }, 'issues', 'detail', { id: 'TEAM-42' }),
    workCacheKey({ id: 'general', path: '/code' }, 'issues', 'detail', { id: 'TEAM-42' }),
  ]
