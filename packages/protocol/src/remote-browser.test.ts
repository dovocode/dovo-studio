...
it('bounds frames, pointer coordinates, scroll bursts and typed text', () => {
  expect(remoteBrowserViewportSchema.safeParse({ width: 390, height: 844 }).success).toBe(true)
  expect(remoteBrowserViewportSchema.safeParse({ width: 9000, height: 844 }).success).toBe(false)
  expect(
    remoteBrowserInputSchema.safeParse({ type: 'pointer', phase: 'down', x: -10, y: 10 }).success,
  ).toBe(false)
  expect(
    remoteBrowserInputSchema.safeParse({ type: 'scroll', x: 1, y: 2, deltaX: 0, deltaY: Infinity })
      .success,
  ).toBe(false)
  expect(
    remoteBrowserInputSchema.safeParse({ type: 'text', text: 'x'.repeat(16001) }).success,
  ).toBe(false)
  expect(
    remoteBrowserInputSchema.safeParse({ type: 'evaluate', expression: 'process.env' }).success,
  ).toBe(false)
})
it('accepts keyboard shortcuts without exposing arbitrary protocol commands', () => {
  for (const key of [
    ...
    expect(remoteBrowserInputSchema.safeParse({ type: 'key', key }).success).toBe(true)
  }
  for (const key of ['', '\n', 'Control+unknown', 'Runtime.evaluate']) {
    expect(remoteBrowserInputSchema.safeParse({ type: 'key', key }).success).toBe(false)
  }
})
...
it('round-trips binary frames and rejects malformed dimensions and headers', async () => {
  ...
  const packet = encodeBrowserFrame({ data, width: 390, height: 844 }, 7)
  expect(decodeBrowserFrame(packet.buffer)).toEqual({ sequence: 7, width: 390, height: 844, data })
