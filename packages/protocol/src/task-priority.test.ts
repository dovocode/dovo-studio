...
const b = { ...task('Beta', '2026-09-02T00:00:00Z'), updatedAt: '2026-09-03T00:00:00Z' }
...
describe('Thread sorting', () => {
  ...
  it('keeps pinned threads first for every sort', () => {
    for (const option of taskSortOptions)
      expect(compareTasks({ ...a, pinned: true }, b, option.id, needsInput, projects)).toBeLessThan(
        0,
      )
  })
  it('has deterministic ties and accepts deleted projects', () => {
    expect(compareTasks(a, { ...a, id: 'other' }, 'project', new Set(), new Map())).toBeLessThan(0)
