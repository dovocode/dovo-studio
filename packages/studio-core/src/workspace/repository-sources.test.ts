...
>
let store: Store
vi.mock('./provider', () => ({ useWorkspace: () => store }))
...
const snapshot = snapshotSchema.parse({
  ...
  workspace: {
    ...
    tasks: [],
    repositories: [
      { id: 'same-repo', name: 'Cached project', path: '/cached/project', branch: 'main' },
    ],
  },
  approvals: [],
  questions: [],
...
const runtime = (address: string, name: string): RuntimeOverview => ({
  profile: runtimeProfile({ address, token: 'fixture-credential-123456789' }, name),
  snapshot,
  connected: true,
  lastSeen: null,
  error: null,
...
beforeEach(() => {
  store = {
    ...
    workspace: {
      ...snapshot.workspace,
      repositories: [{ ...snapshot.workspace.repositories[0], name: 'Unsent rename' }],
    },
    connected: true,
    runtimes: [mac, linux],
    readRuntime: async (profile, _path, _input, schema) => schema.parse({ owner: profile.id }),
...
it('binds each request and read cache to its owner independently of the active host', async () => {
  ...
  expect(await source.request('/api/read', {}, z.object({ owner: z.string() }))).toEqual({
    owner: linux.profile.id,
  })
  store = { ...store, activeRuntimeId: linux.profile.id, workspace: snapshot.workspace }
...
it('changes the content scope when credentials, checkout path or Jira binding changes', () => {
  ...
  const replacement = {
    ...
    profile: {
      ...linux.profile,
      connection: { ...linux.profile.connection, token: 'replacement-credential-123456789' },
    },
  }
  store = { ...store, runtimes: [mac, replacement] }
            ...
            repositories: [
              {
                ...
                path: '/other/project',
                jira: { site: 'https://example.atlassian.net', project: 'NEW' },
              },
            ],
          },
        },
...
it('retains cached offline projects and omits computers without a loaded snapshot', () => {
  store = {
    ...
    runtimes: [
      ...
      { ...runtime('http://new.local:51464', 'New'), snapshot: null },
    ],
  }
