...
const mac = runtimeProfile(
  { address: 'http://mac.local:51464', token: 'fixture-token-123456789' },
  'Mac',
)
const linux = runtimeProfile(
  { address: 'http://linux.local:51464', token: 'fixture-token-987654321' },
  'Linux',
)
...
function setup() {
  ...
  const store: Store = {
    ...
    runtimeRegistry: { version: 1, activeId: mac.id, profiles: [mac, linux] },
    request: async (path, input, schema, method) => {
      requests.push({ path, input, method, owner: mac.id })
    ...
    readRuntime: async (profile, path, input, schema, method) => {
      ...
      requests.push({ path, input, method, owner: profile.id })
      return schema.parse({ ok: true })
    },
...
it('patches only requested metadata with optimistic conflict guards', () => {
  expect(taskRowPatch(task, { title: 'Renamed', pinned: true })).toEqual({
    collection: 'tasks',
    id: task.id,
    changes: {
      title: { before: 'Original title', after: 'Renamed' },
      pinned: { before: null, after: true },
    },
  })
  expect(
    taskRowPatch(
      { ...task, pinned: true, snoozedUntil: '2026-09-24T12:00:00Z' },
      {
        pinned: false,
        snoozedUntil: null,
      },
    ...
    pinned: { before: true, after: false },
    snoozedUntil: { before: '2026-09-24T12:00:00Z', after: null },
  })
  expect(taskRowPatch(task, { title: task.title, harness: undefined }).changes).toEqual({})
})
...
it('uses the task checkout branch and first user text, not a different project checkout or later prompt', () => {
  ...
  expect(taskRowValues({ ...task, messages }, source)).toMatchObject({
    branch: 'main',
    titlePrompt: 'Build the editor',
  })
  expect(taskRowValues({ ...task, execution: 'worktree' }, source).branch).toBe('')
  expect(
    taskRowValues({ ...task, execution: 'worktree', checkoutBranch: 'task/editor' }, source).branch,
  ).toBe('task/editor')
...
it('sends remote task mutations to their captured owner without switching the active device', async () => {
  ...
  await client.request(
    '/api/tasks/viewed',
    { id: task.id, turnId: 'turn', viewed: false, expectedRevision: 2 },
  ...
  expect(requests).toEqual([
    {
      ...
      input: { id: task.id, turnId: 'turn', viewed: false, expectedRevision: 2 },
      method: undefined,
      owner: linux.id,
    },
  ])
...
it('retains the active request guard and refreshes only its own profile', async () => {
  const { store, requests } = setup()
  const client = taskActionClient(store, { ...source, runtimeId: mac.id })
  await client.request('/api/workspace', { id: task.id }, z.object({ ok: z.boolean() }), 'PATCH')
  await client.refresh()
  expect(requests).toEqual([
    { path: '/api/workspace', input: { id: task.id }, method: 'PATCH', owner: mac.id },
  ])
...
it('rejects offline, forgotten or replaced remote owners instead of using the active runtime', async () => {
  ...
  await expect(
    taskActionClient(store, { ...source, online: false }).request('/api/workspace', {}, response),
  ).rejects.toThrow('offline')
  const original = taskActionClient(store, source)
  store.runtimeRegistry = { ...store.runtimeRegistry, profiles: [mac] }
  await expect(
