...
const pageSize = 100
const folderNames = new Intl.Collator(undefined, { numeric: true, sensitivity: 'base' })
export async function listDirectories(value: unknown): Promise<DirectoryPage> {
  const input = directoryRequestSchema.parse(value)
  try {
    ...
    for (const entry of candidates) {
      ...
      if (directory) folders.push({ name: entry.name, path: join(path, entry.name) })
    }
    folders.sort(
      (a, b) =>
    ...
    let ancestor = path
    while (true) {
      breadcrumbs.unshift({ name: basename(ancestor) || ancestor, path: ancestor })
