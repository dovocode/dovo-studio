...
function fixture(
  ...
  const http = new ForgeHttp(
    ...
    async (input, init) => {
      ...
      if (!selected) {
        ...
        else if (path.endsWith('/connectionData'))
          data = {
  ...
  return {
    forge: new AzureForge(http, name),
    calls,
  }
}
...
it('accepts Azure create responses without webUrl or merge metadata while refusing premature mutations', async () => {
  ...
  expect(
    ...
    status: 'created',
    url: `${repository.webUrl}/pullrequest/7`,
  })
  await expect(
    forge.act({
      action: 'close',
