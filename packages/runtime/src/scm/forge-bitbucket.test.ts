...
const repository = {
  ...
  links: {
    ...
    clone: [
      {
        name: 'ssh',
        href: 'git@bitbucket.org:team/studio.git',
      },
      {
        name: 'https',
        href: 'https://dominic@bitbucket.org/team/studio.git',
      },
    ],
  },
}
  ...
  participants: [
    {
      user,
      approved: true,
      participated_on: '2026-09-20T07:00:00Z',
    },
  ],
  links: {
    html: {
...
function fixture(
  ...
  const http = new ForgeHttp(
    ...
    async (input, init) => {
      ...
      if (!selected) {
        ...
        else if (call.url.pathname.endsWith('/comments'))
          data = {
            values: [comment],
          }
  ...
  return {
    forge: new BitbucketForge(http, name),
    calls,
  }
}
it('discovers repositories through the current workspace API, not the retired global API', async () => {
  const { forge, calls } = fixture(({ url }) => {
    ...
    if (url.pathname.endsWith('/repositories/team'))
      return {
        data: {
          values: [repository],
        },
  ...
  const { forge, calls } = fixture(({ url }) =>
    url.pathname.endsWith('/pullrequests')
      ? {
          data: {
            values: [pull],
            next: 'https://attacker.example/pulls',
          },
        }
      : undefined,
  )
...
it('keeps useful detail and unknown counts when optional data fails', async () => {
  const { forge } = fixture(({ url }) =>
    /\/(diff|diffstat|statuses)$/.test(url.pathname)
      ? {
          data: {},
          status: 403,
        }
