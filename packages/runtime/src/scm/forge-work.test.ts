...
it('keeps PRs out of issue results without truncating pagination', async () => {
  ...
  const provider = new GitForgeWork(
    {
      json,
    },
    'github',
...
it('searches beyond the loaded GitHub issues with a fixed repository scope', async () => {
  ...
  const result = await new GitForgeWork(
    {
      json,
    },
    'github',
    'me/app',
  ).issues('open', undefined, 'login repo:other/private')
...
it('renders issues and comments belonging to deleted users', async () => {
  ...
  const detail = await new GitForgeWork(
    {
      json,
    },
    'github',
...
it('uses Forgejo update times and Gitea workflow paths and completion times', async () => {
  ...
  const forgejo = await new GitForgeWork(
    {
      json,
    },
    'forgejo',
  ...
  const gitea = await new GitForgeWork(
    {
      json,
    },
    'gitea',
  ...
  expect(gitea.items[0]).toMatchObject({
...
it('explicitly disables retired native Bitbucket issues', async () => {
  const provider = new BitbucketForgeWork(
    {
      json: vi.fn<WorkHttp['json']>(),
    },
    'me/app',
  )
...
it('rejects unbounded pipeline input counts', () => {
  expect(
    decodeResult(forgePipelineActionSchema, {
      action: 'run',
      definition: 'ci.yml',
      ref: 'main',
...
it.each(['inProgress', 'notStarted', 'postponed', 'cancelling', 'none'])(
  ...
  async (status) => {
    ...
    await expect(
      azureProvider(json).actOnPipeline({
        action: 'rerun',
        id: '5',
      }),
    ).rejects.toMatchObject({
...
it('retries a completed cancelled Azure build once', async () => {
  ...
  await azureProvider(json).actOnPipeline({
    action: 'rerun',
    id: '5',
  })
