...
export function createWorkTaskEffect(s: Services, value: unknown) {
  return runtimeProgram(
    Effect.gen(function* () {
      const input = decode(workTaskInputSchema, value)
      ...
      const jiraSourceId = input.kind === 'issue' ? input.jiraSourceId : undefined
      const request = (operation: string, data: unknown) =>
        jiraSourceId
          ? runtimeOperation(() => s.forgeWork.requestJira(jiraSourceId, operation, data))
          : runtimeOperation(() => s.forgeWork.request(input.repositoryId, operation, data))
      ...
      if (!options[area])
        throw new HttpError(400, `This project's provider does not support ${area}`)
      ...
      if (input.kind === 'issue') {
        ...
        if (issue.id !== input.id || issue.url !== input.url || issue.revision !== input.revision)
          throw new HttpError(
            409,
            'This issue changed. Refresh its details before creating a task.',
          )
        source = {
          kind: 'issue',
        ...
        const detail = decode(
          forgePipelineDetailSchema,
          yield* request('pipelines/detail', {
            id: input.id,
            refresh: true,
          }),
        )
        ...
        source = decode(taskWorkItemSchema, {
          kind: 'pipeline',
          provider: options.provider,
          id: run.id,
          url: run.url,
...
export function createWorkTask(s: Services, value: unknown) {
  return runClientEffect(createWorkTaskEffect(s, value))
}
