...
function fixture(
          ...
          statuses: [
            {
              context: 'build',
              status: 'failure',
              target_url: `${repo.html_url}/actions/runs/2`,
            },
          ],
        })
  ...
  return {
    adapter: new GiteaForge(http, repository),
    requests,
  }
}
const lineComment = {
...
describe('Forgejo and Gitea', () => {
  it('discovers repositories without a binding and keeps custom server subpaths', async () => {
    ...
    await expect(adapter.repository()).rejects.toMatchObject({
      status: 400,
    })
  })
  ...
  it('posts discussion and closes or reopens through the correct resources', async () => {
    ...
    expect(writes(requests)).toEqual([
      ...
      {
        path: '/repos/team/project/pulls/7',
        method: 'PATCH',
        body: {
          state: 'closed',
        },
