...
const parsedIssue = decode(forgeIssueDetailSchema, {
  issue: {
    id: 'TEAM-1',
    title: 'General work',
...
it('keeps account/source cache identities distinct and refuses stale or mismatched linking', async () => {
  ...
  s.store.update((w) => ({
    ...w,
    jiraSources: [source, another],
    repositories: [repository],
  }))
  await s.forgeWork.requestJira(source.id, 'issues/list', {
    query: 'login',
  })
  await s.forgeWork.requestJira(source.id, 'issues/list', {
    query: 'login',
  })
  await s.forgeWork.requestJira(another.id, 'issues/list', {
    query: 'login',
  })
...
it('creates a task in the chosen Dovo project while retaining its separate Jira source', async () => {
  ...
  expect(
    await post('work/task', {
      ...input,
      jiraSourceId: 'another',
    }),
  ).toMatchObject({
    status: 409,
  })
...
it('restores repository native issues when legacy Jira bindings migrate', async () => {
  ...
  expect(s.store.get().jiraSources).toMatchObject([
    {
      site: source.site,
      project: source.project,
    },
...
it('preserves independent source identity when renaming and removes only its app links', async () => {
  ...
  expect(
    await post('jira/sources/remove', {
      sourceId: source.id,
    }),
  ).toMatchObject({
    status: 200,
