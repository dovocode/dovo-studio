...
export class AgentRegistry {
  ...
  constructor(private settings: () => CommandSettings = () => commandsSchema.parse({})) {
    const providers = [
    ...
    for (const [id, load] of providers)
      this.host.register({
        ...
        activate: async (context) => {
          this.adapters.set(id, await load())
  ...
  async get(provider: Agent['provider']) {
    await this.host.activate(`dovo.provider.${provider}`)
    const adapter = this.adapters.get(provider)
    if (!adapter) throw new Error('Provider failed to activate')
    const models = adapter.models
    return {
      run: (run) => adapter.run({ ...run, agent: this.configure(run.agent) }),
      probe: (agent) => adapter.probe(this.configure(agent)),
      ...(models ? { models: (agent: AgentDiscovery) => models(this.configure(agent)) } : {}),
