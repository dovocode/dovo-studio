...
it.each([false, true])('streams assistant text with text-only tool policy %s', async (textOnly) => {
  ...
  const server = createServer((request, response) => {
    ...
    request.on('end', () => requests.push({ path, body }))
    response.setHeader('Content-Type', 'application/json')
    if (path === '/mcp') {
      request.on('end', () =>
        response.end(JSON.stringify({ [JSON.parse(body).name]: { status: 'connected' } })),
      )
      return
    }
    if (path === '/session') {
    ...
    if (path === '/session/session/message') {
      ...
      void listening.then(() => {
        ...
        emit('message.part.delta', { sessionID: 'unrelated', field: 'text', delta: 'Another task' })
        for (const [id, type, text] of [
          ...
          emit('message.part.updated', {
            sessionID: 'session',
            part: { id, type, sessionID: 'session', messageID: 'message', text: '' },
            time: Date.now(),
          })
  ...
  try {
    ...
    expect(registration).toMatchObject(
      textOnly
        ...
        : {
            name: managedName,
            config: { type: 'local', command: ['node', 'docs.js'], enabled: true },
          },
    )
    expect(
    ...
    expect(session.permission).toEqual([
      ...
      { permission: 'dovo_*', pattern: '*', action: 'deny' },
      ...(textOnly ? [] : [{ permission: registration.name + '_*', pattern: '*', action: 'deny' }]),
    ])
