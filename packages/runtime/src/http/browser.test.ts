...
it('checks task ownership and credentials before launching, isolates browser tickets, and detaches on revocation', async () => {
  ...
  socket.send(JSON.stringify({ type: 'text', text: 'remote input' }))
  await vi.waitFor(() =>
    expect(input).toHaveBeenCalledWith(
      'task',
      { type: 'text', text: 'remote input' },
      expect.any(Function),
    ),
  )
  const frames: string[] = []
  socket.on('message', (raw) => {
    const message = remoteBrowserMessageSchema.parse(
      JSON.parse(
        (Array.isArray(raw)
          ? Buffer.concat(raw)
          : Buffer.isBuffer(raw)
  ...
  publish({ type: 'frame', data: Buffer.from('stale'), width: 390, height: 844 })
  buffered.mockReturnValue(0)
  publish({ type: 'frame', data: Buffer.from('latest'), width: 390, height: 844 })
  ...
  binary.on('message', (raw, isBinary) => {
    ...
    received.push({ sequence: frame.sequence, text: Buffer.from(frame.data).toString() })
  })
  ...
  for (const text of ['first', 'second', 'old-pending', 'newest'])
    publishBinary({ type: 'frame', width: 390, height: 844, data: Buffer.from(text) })
  await vi.waitFor(() =>
  ...
  binary.send(JSON.stringify({ type: 'frameAck', sequence: 1 }))
  await vi.waitFor(() => expect(received.at(-1)).toEqual({ sequence: 3, text: 'newest' }))
  expect(received).toHaveLength(3)
  expect(input).toHaveBeenCalledTimes(1)
  const invalidAck = once(binary, 'close')
  binary.send(JSON.stringify({ type: 'frameAck', sequence: 99 }))
